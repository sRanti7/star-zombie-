...
ora = loadSound("jo.mp3")
shooterImg.playing = true;
shooter_shooting.playing = true;
...
if(keyWentDown("space")){
 setTimeout(player.changeAnimation("hit"),50000)
...
//player goes back to original standing image once we stop pressing the space bar
  ...
  //player.addAnimation("start",shooterImg)
  setTimeout(player.changeAnimation("start"),10000)
  player.scale = 1.2 
}
/*else {
  setTimeout(player.changeAnimation("start"),10000);
...
function disparo_(){      
setTimeout( disparo = createSprite(player.x, width/2 ,50 , 20), 5000 );
disparo.y = player.y;
disparo.lifetime= 20
